...
def experiment_svm(input_path_training):
    ...
    classifier = Pipeline([
        ...
        ('clf', OneVsRestClassifier(SVC(cache_size=3500, class_weight='balanced', kernel="linear", C=10, verbose=1))),
    ])
    classifier.fit(X, y)
...
def experiment_svm_cv(input_path_training):
    ...
    classifier = Pipeline([
        ...
        ('clf', OneVsRestClassifier(SVC(cache_size=3500, class_weight='balanced'))),
    ])
