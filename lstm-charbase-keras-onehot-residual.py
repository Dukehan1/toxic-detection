...
from keras.layers import Input, Dense, Embedding, SpatialDropout1D, concatenate, Bidirectional, \
    GlobalAveragePooling1D, GlobalMaxPooling1D, Conv1D, LSTM, Add, GRU, Activation
...
def experiment(dev_id, model_dir):
    ...
    def get_model():
        ...
        x = Bidirectional(GRU(int(sys.argv[1].split('_')[2]), return_sequences=True,
                              recurrent_dropout=float(sys.argv[1].split('_')[0])))(x_in)
        x = Bidirectional(GRU(int(sys.argv[1].split('_')[2]), return_sequences=True,
                              recurrent_dropout=float(sys.argv[1].split('_')[1])))(x)
        x_raw = Dense(int(sys.argv[1].split('_')[2])*2, activation='relu', kernel_initializer='glorot_normal')(x_in)
        x = Add()([x, x_raw]) 
        x = Activation('relu')(x)
...
if __name__ == "__main__":
    ...
    model_dir = os.path.join(os.path.abspath('.'), 'onehot_' + sys.argv[1])
    mkdir_p(model_dir)

    experiment(10, model_dir)
