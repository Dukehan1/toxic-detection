...
class Config(object):
    ...
    char_embed_size = CHAR_EMBEDDING_DIM
    batch_size = 128
    n_epochs = 4
    lr = 0.001
    dropout = 0.5
...
class GRU_CNNModel(Model):
    def add_placeholders(self):
        ...
        self.inputs_char_placeholder = tf.placeholder(tf.int32,
                                                     (None, self.config.max_length, self.config.max_word_length))
        self.labels_placeholder = tf.placeholder(tf.float32, (None, self.config.label_num))
        self.dropout_placeholder = tf.placeholder(tf.float32)

    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        feed_dict = {
            self.inputs_placeholder: inputs_batch.word,
            self.inputs_char_placeholder: inputs_batch.char,
    ...
    def add_prediction_op(self):
        ...
        word = tf.nn.embedding_lookup(word_embeddings, self.inputs_placeholder)
        word = tf.nn.dropout(word, self.dropout_placeholder)

        char_embeddings = tf.get_variable('CharEmb', (self.config.char_vocab_size, self.config.char_embed_size), dtype=tf.float32)
        chars = tf.nn.embedding_lookup(char_embeddings, self.inputs_char_placeholder)
        chars = tf.nn.dropout(chars, self.dropout_placeholder)

        char_gru_fw_cell = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.config.char_rnn_hidden_size / 2),
                                                         output_keep
        ...
        gru_bw_cell = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.config.hidden_size / 2),
                                                     output_keep_prob=self.dropout_placeholder)
        outputs, _ = tf.nn.bidirectional_dynamic_rnn(gru_fw_cell, gru_bw_cell, word, dtype=tf.float32)
        ...
        U = tf.get_variable('U', (self.config.num_filters * 2, self.config.label_num),
                            tf.float32, tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable('b2', (self.config.label_num,),
                             tf.float32, tf.contrib.layers.xavier_initializer())

        preds = tf.matmul(h_pool_flat, U) + b2
